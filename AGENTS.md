...
JavaScript implementation of Clyde using the [OpenAI Agents SDK](https://openai.github.io/openai-agents-js/) (`@openai/agents`).
...
- **ClydeDeps** — verify constructor stores all properties

## Architecture

### Agent Layer

The agent is defined in `agent/clyde-agent.js` using the OpenAI Agents SDK:
...
`agent/deps.js` defines a `ClydeDeps` class that holds `client`, `userId`, `channelId`, `threadTs`, `messageTs`, and `userToken`. This is passed as the `context` parameter to `run()` and is available in tool `execute` functions via `context.context`.

### Slack MCP Server

`clyde-agent.js` exports a `runClyde(inputItems, deps)` function that wraps agent execution. When `deps.userToken` is present, it connects to the Slack MCP Server (`https://mcp.slack.com/mcp`) via `MCPServerStreamableHttp`, clones the agent with the MCP server attached, and runs it. The MCP connection is always closed in a `finally` block.
