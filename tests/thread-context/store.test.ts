...
describe('ConversationStore', () => {
  ...
  it('keeps different threads independent', () => {
    const msgs1 = [{ role: 'user', content: 'hello' } as any];
    const msgs2 = [{ role: 'user', content: 'world' } as any];
  ...
  it('expires entries after TTL', async () => {
    ...
    shortStore.setHistory('C1', 'T1', [{ role: 'user', content: 'hi' } as any]);
  ...
  it('evicts oldest entries when max is exceeded', () => {
    ...
    smallStore.setHistory('C1', 'T1', [{ role: 'user', content: '1' } as any]);
    smallStore.setHistory('C1', 'T2', [{ role: 'user', content: '2' } as any]);
    smallStore.setHistory('C1', 'T3', [{ role: 'user', content: '3' } as any]);
  ...
  it('overwrites existing key', () => {
    store.setHistory('C1', 'T1', [{ role: 'user', content: 'old' } as any]);
    store.setHistory('C1', 'T1', [{ role: 'user', content: 'new' } as any]);
